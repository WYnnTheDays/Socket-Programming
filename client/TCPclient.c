...
int recvFromServer(int sockfd, char* buf, int buf_size)
{
    ...
    {
        fprintf(stderr,"[CLIENT] no message received.\n");
        return -1;
    }
    else
    {
...
void process(FILE *fp, int sockfd)
{
    ...
    while (getMessage(sendline, BUFFSIZE, fp) != NULL)
    {
        ...
        {
            fprintf(stderr,"[CLIENT] server terminated.\n");
            return;
        }
        else if (numbytes > 0)
        {
...
int main(int argc,char* argv[])
{
    ...
    fprintf(stderr,"[CLIENT] server addr: %s, port: %u\n", inet_ntoa(server.sin_addr), ntohs(server.sin_port));

    if (connect(sock, (struct sockaddr*)&server, sizeof(server)) < 0)
        errexit("connect to server failed: %s\n", strerror(errno));

    fprintf(stderr,"[CLIENT] connected to server %s\n", inet_ntoa(server.sin_addr));
